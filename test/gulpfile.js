...
gulp.task('clean', function (cb) {
  del(['.tmp', 'dist','bower_components/othertree-client'], cb);
});

gulp.task('copy-othertree-client', function (cb) {
  var imports= gulp.src([
    '../othertree-client.html','../bower.json','../project.json'         
   ]).pipe(gulp.dest('bower_components/othertree-client/'));
   
   var dists= gulp.src([
    '../dist/**/*'        
   ]).pipe(gulp.dest('bower_components/othertree-client/dist'));
   
   return merge(imports, dists);
...
gulp.task('serve', ['copy-othertree-client','styles', 'elements', 'images','js'], function () {
  browserSync({
    port: 5000,
    notify: false,
    logPrefix: 'PSK',
  ...
  gulp.watch(['../dist/**/*.js'], 'copy-othertree-client');
